...
public class Board {
    ...
    public int manhattan() {
        LinkedList<Position> positionLinkedList = new LinkedList<>(positions); //The location of the elements in the LinkedList + 1 (positionLinkedList.get(x)) corresponds the desired value of that position in the solution
        int manhattanValue = 0;
        for (Position pos : positions) {
            int token = getField(pos);
            if (token != Board.BLANK) {
                Position solutionPosition = positionLinkedList.get(token - 1); //Token -1 so the correct element is chosen
                int xVal = Math.abs(pos.x - solutionPosition.x);
                int yVal = Math.abs(pos.y - solutionPosition.y);
                manhattanValue += xVal + yVal;
            }
        }
    ...
    public static void main(String[] args) {
        String filename = "samples/board-manhattan1.txt";
